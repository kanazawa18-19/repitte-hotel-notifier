...
なぜ要るか
  monitor.py は 2026-09-10 にページング対応済みだが、それ以前の取得は
  limit=50 の1回だけで、1回の取得窓に51件以上あると**古い方が二度と読まれなかった**。
  そのうえで最新のtsをstateに書くため、落ちた分は永久に復旧しない。
  過去に落ちた分が無いかは、実データを数えないと分からない。

  取得窓の広さ（cronの見た目と全く違う）と、50件を超えた回数の実測は
  AGENTS.md の「実測値」表が正本。ここには数値を書き足さない。
